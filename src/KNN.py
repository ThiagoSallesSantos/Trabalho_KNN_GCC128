...
class KNN:
    ...
    def __init__(self, vizinhos = 1, metodo = "media"):
        self._vizinhos = vizinhos if vizinhos >= 1 else 1 ## Armazena a quantidade de "prototipos"
        ...
        self._metodo = metodo if metodo in self._dict_metodos.keys() else "media" ## Armazerna qual metodo para achar prototipo usar
        self._prototipos = dict({}) ## Armazena os prototipos
        self._dict_individuos = dict({}) ## Armazena os individuos

    def _distancia_euclidiana(self, individuo_A, individuo_B): ## Calcula distância Euclidiana
    ...
    def _media_min_max(self, matriz): ## Calcula a média usando os valores min e max de cada atributo
    ...
    def treinamento(self, X_treino, Y_treino): ## Realiza o treinamento
        ...
        self._dict_individuos = dict({grupo[0] : np.array(grupo[1][lista_atributos].values.tolist()) for grupo in dados.groupby(by="Resultados")})
        for grupo in self._dict_individuos.keys(): ## For para criar os prototipos
            prototipo_ficticio = self._dict_metodos[self._metodo](self._dict_individuos[grupo]) ## Cria um prototipo ficticio, usando o metodo escolhido
            lista_individuos = self._dict_individuos[grupo].copy()
            for individuo in lista_individuos:
                list(individuo).append(self._distancia_euclidiana(prototipo_ficticio, individuo)) ## Calcula a distância euclidiana, entre prototipo ficticio, para todos os individuos do conjunto
            self._prototipos.update(dict({grupo : lista_individuos[lista_individuos[:, -1].argsort()][:self._vizinhos]})) ## Salva os protipos
        ...
        for individuo in X_teste.values.tolist(): ## Percorre os individuos que devem ser preditos
            ...
            for grupo in self._prototipos.keys():
                ...
                for prototipo in self._prototipos[grupo]:
                    resultado_preliminar[grupo].append(self._distancia_euclidiana(prototipo, individuo)) ## Calcula a distância euclidiana entre os prototipos para os individuos
                resultado_preliminar[grupo] = min(resultado_preliminar[grupo]) ## Pego a menor distância encontrada para o conjunto
            resultados.append(min(resultado_preliminar, key=resultado_preliminar.get)) ## Pego qual conjunto foi menor
        return resultados
